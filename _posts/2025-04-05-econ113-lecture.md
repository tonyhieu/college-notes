...
- **Evolutionary Stable Strategy (ESS)**: A strategy $\sigma^*$ that two various requirements
    - $\sigma^*$ is a NE such that $u(\sigma^{*}, \sigma^{*}) \geq u(\sigma', \sigma^*), \forall \sigma' \neq \sigma^*$
    - $\sigma^*$ is impervious to invasion such that if $u(\sigma^{*}, \sigma^{*}) = u(\sigma', \sigma^*)$, then $u(\sigma^{*}, \sigma') > u(\sigma', \sigma')$
...
- Adding elimination into group models allows you to show that cooperation can increase over time under the RD
    ...
    - With random winners via a contest winner function, cooperation is expected to increase and shows that in-group cooperation is selected over defection
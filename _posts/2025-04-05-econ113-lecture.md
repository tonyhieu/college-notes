...
- **Evolutionary Stable Strategy (ESS)**: A strategy $\sigma^*$ that two various requirements
    - $$\text{$\sigma^*$ is a NE such that $u(\sigma^{*}, \sigma^{*}) \geq u(\sigma', \sigma^*), \forall \sigma' \neq \sigma^*$}$$
    - $$\text{$\sigma^*$ is impervious to invasion such that if $u(\sigma^{*}, \sigma^{*}) = u(\sigma', \sigma^*)$, then $u(\sigma^{*}, \sigma') > u(\sigma', \sigma')$}$$
